...
if [ "$BLACKLIST_COUNTRY" != "" ] || [ "$WHITELIST_COUNTRY" != "" ] ; then
	...
	echo "0 23 2 * * /opt/scripts/geoip.sh" >> /etc/crontabs/root
...
if [ "$(has_value BLOCK_USER_AGENT yes)" != "" ] ; then
	...
	echo "30 0 * * * /opt/scripts/user-agents.sh" >> /etc/crontabs/root
...
if [ "$(has_value BLOCK_REFERRER yes)" != "" ] ; then
	...
	echo "0 1 * * * /opt/scripts/referrers.sh" >> /etc/crontabs/root
...
if [ "$(has_value BLOCK_TOR_EXIT_NODE yes)" != "" ] ; then
	echo "15 * * * * /opt/scripts/exit-nodes.sh" >> /etc/crontabs/root
...
if [ "$(has_value BLOCK_PROXIES yes)" != "" ] ; then
	echo "30 1 * * * /opt/scripts/proxies.sh" >> /etc/crontabs/root
...
if [ "$(has_value BLOCK_ABUSERS yes)" != "" ] ; then
	echo "0 2 * * * /opt/scripts/abusers.sh" >> /etc/crontabs/root
...
if [ "$(has_value USE_CLAMAV_UPLOAD yes)" != "" ] || [ "$USE_CLAMAV_SCAN" = "yes" ] ; then
	...
	echo "30 2 * * * /usr/bin/freshclam > /dev/null 2>&1" >> /etc/crontabs/root
fi
if [ "$USE_CLAMAV_SCAN" = "yes" ] ; then
	if [ "$USE_CLAMAV_SCAN_REMOVE" = "yes" ] ; then
		echo "45 */1 * * * /usr/bin/clamscan -r -i --no-summary --remove / >> /var/log/clamav.log 2>&1" >> /etc/crontabs/root
	else
		echo "45 */1 * * * /usr/bin/clamscan -r -i --no-summary / >> /var/log/clamav.log 2>&1" >> /etc/crontabs/root
	fi
fi
...
echo "0 3 * * * /opt/scripts/logrotate.sh > /dev/null 2>&1" >> /etc/crontabs/root
